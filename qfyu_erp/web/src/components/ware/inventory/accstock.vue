...
<div>
    ...
    <Modal width="390" v-model="add.status" :mask-closable="false" footer-hide title="盘点选项" class-name="acc-lst">
      <Form :label-width="85">
        <FormItem label="盘点仓库">
          <Input icon="ios-more" :value="add.ware_title" style="cursor: pointer;" readonly @on-click="show_choware"></Input>
        </FormItem>
        <FormItem label="货位">
          <Input icon="ios-more" :value="add.pos_code" style="cursor: pointer;" readonly @on-click="openpos"></Input>
        </FormItem>
        <FormItem label="商品大类">
          <Select v-model="add.category_id" style="width:100%">
            <Option v-for="(item,key) in add_cfg.category" :value="item.id" :key="key">{{item.title}}</Option>
          </Select>
        </FormItem>
        <FormItem label="功能类别">
          <Input icon="ios-more" :value="add.function" style="cursor: pointer;" readonly @on-click="show_selgfn"></Input>
        </FormItem>
        <FormItem label="">
    ...
    export default {
      ...
      methods:{
           ...
           */
            ...
            init:function(){
                _this.loading=true;
                _this.common.http({url:'w/bookList',data:{}}).then(function(res){
                    _this.loading=false;
                    if(res.status==1){
                        _this.list=res.data;
                    }
             ...
             */
            add_new:function (){
              this.add={status:true,ware_code:'',ware_title:'',pos_code:'',category_id:0,function_id:0,function:'',noqty:0};
            },
            /**
             * 打开仓库选择
             */
           ...
           */
            ...
            aedit:function(){
              ...
              localStorage.setItem('accstock_editid',this.list[this.selectIndex].bill_no);
              this.toUrl('edit_accstock');
            },
          /**
           * 提交新增结存单
           */
          sub_add:function (){
            ...
            var data={ware_code:this.add.ware_code,pos_code:this.add.pos_code,category_id:this.add.category_id,function_id:this.add.function_id,noqty:this.add.noqty};
            _this.common.http({url:'w/addAccstock',data:data}).then(function(res){
              _this.$Spin.hide();
              if(res.status==1){
                _this.add.status=false;
                localStorage.setItem('accstock_editid',res.bill_no);
                _this.toUrl('edit_accstock');
              }else{
                _this.$Modal.error({
                  title: '提示',
                  content: res.msg
          ...
          check_acc:function (){
            ...
            if(transfer){
              this.alertMsg('已审核的结存单不能重复审核','error');
              return false;
            }
            _this.$Modal.confirm({
              title:'提示',
              ...
              onOk:function(){
                _this.$Spin.show();
                _this.common.http({url:'w/checkAccstock',data:{bill_no:bill_no}}).then(function(res){
                  _this.$Spin.hide();
                  if(res.status==1){
                    _this.init();
          ...
          del_acc:function (){
            ...
            var bill_no=this.list[this.selectIndex].bill_no,transfer=this.list[this.selectIndex].transfer;
            if(transfer){
              this.alertMsg('已审核的结存单不能删除','error');
              return false;
            }
            _this.$Modal.confirm({
              title:'提示',
              ...
              onOk:function(){
                _this.$Spin.show();
                _this.common.http({url:'w/delAccstock',data:{bill_no:bill_no}}).then(function(res){
                  _this.$Spin.hide();
                  if(res.status==1){
                    _this.init();
                    _this.alertMsg(res.msg);
