        ...
        <div class="ser-form">
          <Form :label-width="90" class="orderform">
            ...
            <Row>
              <Col span="6">
                <FormItem label="">
                  <Checkbox v-model="initCfg.setTqty" :true-value="1" :false-value="0">将账面库存设为实际库存</Checkbox>
                </FormItem>
              </Col>
              <Col span="10">
                <FormItem label="明细添加方式：" :label-width="110">
                  <RadioGroup v-model="initCfg.dtype" @on-change="change_dtype">
                    <Radio :label="1">添加盘盈记录</Radio>
                    <Radio :label="2">调出库存记录</Radio>
                  </RadioGroup>
                </FormItem>
              </Col>
            </Row>
      ...
      <div slot="table-list">
        <Table size="small" :row-class-name="rowClassName" :columns="columns" :data="list" border class="table-m" :height="height" :loading="loading" @on-row-click="selectRowIndex">
          <template slot-scope="row" slot="pos_code">
            <Input v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" icon="ios-more" :value="row.row.pos_code" readonly style="cursor: pointer;" @on-click="openpos(row.index)" :class="'pos_code'+row.row.index"></Input>
          ...
          <template slot-scope="row" slot="price">
            <input class="ivu-input ivu-input-default" maxlength="16" v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" v-model="list[row.index].price" @input="filter_price(row.index)" :class="'price'+row.row.index" />
            <span v-else>{{row.row.price}}</span>
          </template>
          <template slot-scope="row" slot="rqty">
            <input class="ivu-input ivu-input-default" maxlength="15" v-model="list[row.index].rqty" v-if="canEdit && row.row.dtype!=2" @input="filter_rqty(row.index)" :class="'rqty'+row.row.index" />
            <span v-else>{{row.row.rqty}}</span>
          </template>
          <template slot-scope="row" slot="batch_no">
            <Input v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" maxlength="30" v-model="list[row.index].batch_no" :class="'batch_no'+row.row.index"></Input>
            <span v-else>{{row.row.batch_no}}</span>
          </template>
          <template slot-scope="row" slot="sup_code">
            <Input v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" icon="ios-more" :value="row.row.sup_code" readonly style="cursor: pointer;" @on-click="openprov('',row.index)" :class="'sup_code'+row.row.index"></Input>
            <span v-else>{{row.row.sup_code}}</span>
          </template>
          <template slot-scope="row" slot="prod_date">
            <DatePicker v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" type="date" :editable="false" :clearable="false" v-model="list[row.index].prod_date" :class="'prod_date'+row.row.index"></DatePicker>
            <span v-else>{{row.row.prod_date}}</span>
          </template>
          <template slot-scope="row" slot="valid_date">
            <DatePicker v-if="canEdit && !row.row.stock_id && row.row.dtype!=2" type="date" :editable="false" :clearable="false" v-model="list[row.index].valid_date" :class="'valid_date'+row.row.index"></DatePicker>
            <span v-else>{{row.row.valid_date}}</span>
          </template>
          <template slot-scope="row" slot="remark">
            <Input v-if="canEdit && row.row.dtype!=2" v-model="list[row.index].remark" maxlength="150"></Input>
...
<script>
    ...
    export default {
        ...
        data(){
           return {
              id:localStorage.getItem('acconline_editid'),
        ...
        beforeRouteEnter(to, from, next) {
          next(vm => {
            if(from.name == 'immediate'){
              vm.id = localStorage.getItem('acconline_editid');
            }
          });
        },
        watch:{
        ...
        methods:{
          ...
          selectRowIndex:function(data,index){
            ...
            init:function(){
                _this.loading=true;
                _this.common.http({url:'w/editAcconline',data:{id:this.id}}).then(function(res){
                    _this.loading=false;
                    if(res.status==1){
                      _this.data=res.data;
                      _this.list=res.list;
          ...
          add_line:function (){
            ...
            if(this.list.length==0){
              this.list.push({item_no:1,goods_code:null,rqty:null,stock_id:0,remark:'',isadd:1,dtype:this.initCfg.dtype});
            }else{
              if(this.check_complete()){
                var last_key=this.list.length-1,item_no=parseInt(this.list[last_key].item_no)+1;
                this.list.push({item_no:item_no,goods_code:null,rqty:null,stock_id:0,isadd:1,dtype:this.initCfg.dtype});
              }
            }
          },
          /**
           ...
           * 修改 详情添加方式
           */
          change_dtype:function (dtype){
            var last_key=this.list.length-1;
            if(this.list.length>0) this.list[last_key].dtype=dtype;
          },
          /**
           * 打开货位选择
           */
          ...
          confirmGoods:function (e){
            if(this.list[this.selectIndex].dtype==1){
              ...
              _this.loading=true;
              _this.common.http({url:'w/imStockList',data:{ware_code:this.data.ware_code,goods_code:e.goods_code,maxkey:maxkey,tqty:this.initCfg.setTqty}}).then(function(res){
                _this.loading=false;
                if(res.status==1){
          ...
          uprel:function (){
            ...
            this.$Spin.show();
            _this.common.http({url:'w/upAccOnline',data:{data:this.data,list:this.list}}).then(function(res){
              _this.$Spin.hide();
              if(res.status==1){
                _this.canEdit=false;
                localStorage.setItem('acconline_editid',res.bill_no);
          ...
          check_acc:function (){
            ...
            _this.$Modal.confirm({
              ...
              onOk:function(){
                _this.$Spin.show();
                _this.common.http({url:'w/checkAcconline',data:{bill_no:_this.id}}).then(function(res){
                  _this.$Spin.hide();
                  if(res.status==1){
                    _this.canEdit=false;
                    _this.init();
                    ...
                    for(var bi in res.erritem){
                      for (var i in _this.list){
                        if(res.erritem[bi]==_this.list[i].item_no){
                          _this.err.list.push(_this.list[i]);
                          break;
                        }
                      }
                    }
          ...
          del_errlist:function (){
            var erritem=[],err=_this.err.list;
            for (var i in err){
              erritem.push(err[i].item_no);
            }
            _this.$Spin.show();
            _this.common.http({url:'w/delAccerr',data:{bill_no:_this.id,erritem:erritem}}).then(function(res){
              _this.$Spin.hide();
              if(res.status==1){
