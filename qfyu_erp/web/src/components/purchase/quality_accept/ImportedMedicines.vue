  ...
  export default {
    ...
    data:function () {
      return{
        type:2,
        orderid:null,
        Breadcrumb:[{title:'采购管理'},{title:'质量验收'},{title:'进口药品质量验收'}],
        editIndex:-1,
        editType:0,
        editTypeTitle:'',
        height:0,
    ...
    activated() {
      this.orderid = localStorage.getItem("edit_imported_orderid");
    },
    created() {
    ...
    methods:{
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("edit_imported_orderid")
                if (res.orderid){
                  localStorage.setItem("edit_imported_orderid",res.orderid);
      ...
      out(){
        this.delView('ImportedMedicines');
      },
    }
  }
</script>
