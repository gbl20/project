    ...
    <split-none :Breadcrumb="Breadcrumb" @DHeight="dHeight">
      <div slot="form-ser">
        <div class="nav-menu border-b">
          ...
          <Button @click="del" :disabled="formData.status?true:false"><span class="icon iconfont">&#xe6a5;</span>删除
          </Button>
          ...
          <Button @click="reduction" :disabled="(!formData.status || editType)?true:false"><span class="icon iconfont">&#xe6a2;</span>还原
          </Button>
          <Divider type="vertical"/>
        ...
        <Form ref="FormRef" :rules="ruleInline" :model="formData" :label-width="80" style="width: 1200px;padding-top: 5px;"
          ...
          <Row>
            ...
            <Col span="3">
              <FormItem label="状态">
                <div class="state " :class="formData.status==1?'success-bg-text':'warning-bg-text'">
                  {{formData.status==1?'已审核':'未审核'}}
                </div>
              </FormItem>
            </Col>
          </Row>
  ...
  export default {
    ...
    data: function () {
      return {
        type: 5,
        orderid: null,
        Breadcrumb: [{title: '采购管理'}, {title: '质量验收'}, {title: '特殊药品质量验收'}],
        editIndex:-1,
        editType: 0,
        editTypeTitle: '',
        height: 0,
    ...
    activated() {
      this.orderid = localStorage.getItem("edit_special_orderid");
    },
    created() {
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("edit_special_orderid")
                if (res.orderid){
                  localStorage.setItem("edit_special_orderid",res.orderid);
