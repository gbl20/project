    ...
    <split-none :Breadcrumb="Breadcrumb" @DHeight="dHeight">
      <div slot="form-ser">
        <div class="nav-menu border-b">
          ...
          <Button :disabled="(formData.status || editType==-1)?true:false" @click="save"><span class="icon iconfont">&#xe792;</span>保存</Button>
        ...
        <Form  ref="FormRef" :rules="ruleInline" :model="formData" :label-width="80" style="width: 1200px;padding-top: 5px;" :disabled="editType==-1?true:false">
          <Row>
            <Col span="5">
              <FormItem label="验收单号">
  ...
  export default {
    ...
    data:function () {
      return{
        type:3,
        orderid:null,
        Breadcrumb:[{title:'采购管理'},{title:'质量验收'},{title:'医疗器械质量验收'}],
        editIndex:-1,
        editType:0,
        editTypeTitle:'',
        height:0,
    ...
    activated() {
      this.orderid = localStorage.getItem("edit_instruments_orderid");
    },
    created() {
    ...
    methods:{
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("edit_instruments_orderid")
                if (res.orderid){
                  localStorage.setItem("edit_instruments_orderid",res.orderid);
      ...
      out(){
        this.delView('InstrumentsMedical');
      },
    }
  }
</script>
