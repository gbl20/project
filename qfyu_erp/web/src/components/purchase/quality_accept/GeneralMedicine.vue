    ...
    <split-none :Breadcrumb="Breadcrumb" :editTypeTitle="editTypeTitle" @DHeight="dHeight">
      <div slot="form-ser">
        <div class="nav-menu border-b">
          <Button @click="editData" :disabled="formData.status?true:false"><span class="icon iconfont">&#xe791;</span>修改</Button>
        ...
        <Form ref="FormRef" :model="formData" :rules="ruleInline" :label-width="80" style="width: 1200px;padding-top: 5px;" :disabled="!editType?true:false">
          <Row>
            <Col span="5">
              <FormItem label="验收单号">
              ...
              <FormItem label="来货日期">
                <DatePicker format="yyyy年MM月dd日" type="date" v-model="formData.delivery_date" disabled></DatePicker>
              </FormItem>
              <FormItem label="验收日期">
                <DatePicker format="yyyy年MM月dd日" type="date" v-model="formData.accept_date" disabled></DatePicker>
              </FormItem>
              <FormItem label="总金额">
            ...
            <Col span="3">
              <FormItem label="状态">
                <div class="state " :class="formData.status?'success-bg-text':'warning-bg-text'">{{formData.status?'已审核':'未审核'}}</div>
              </FormItem>
            </Col>
          </Row>
        </Form>
  ...
  import toUrl from "@/assets/purchase/to_type_url.js"
    ...
    data:function () {
      return{
        type:1,
        orderid:null,
        Breadcrumb:[{title:'采购管理'},{title:'质量验收'},{title:'普通药品质量验收'}],
        editIndex:-1,
        editType:0,
        editTypeTitle:'',
        height:0,
    ...
    activated() {
      this.orderid = localStorage.getItem("edit_general_orderid");
    },
    created() {
    ...
    methods:{
      ...
      save:function(){
        if(this.editType==-1) return false;
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("edit_general_orderid")
                if (res.orderid){
                  localStorage.setItem("edit_general_orderid",res.orderid);
      ...
      editData:function(){
        if (this.formData.status) return this.modalMsg('当前单据已经审核，不能进行修改操作！')
        this.editType = 1;
