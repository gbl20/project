          ...
          <Row>
            ...
            <Col span="10">
              ...
              <Row>
                <Col span="12">
                  <FormItem label="创建人">
                    <Input :value="$store.state.emp[formData.create_code]" disabled></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="创建日期">
  ...
  export default {
    ...
    data:function () {
      return{
        type:4,
        orderid:null,
        Breadcrumb:[{title:'采购管理'},{title:'质量验收'},{title:'中药饮片质量验收'}],
        editIndex:-1,
        editType:0,
        editTypeTitle:'',
        height:0,
    ...
    activated() {
      this.orderid = localStorage.getItem("edit_chinese_orderid");
    },
    created() {
    ...
    methods:{
      ...
      init:function(){
        this.common.http({
          ...
          if (res.status){
            ...
            _this.formData = res.form?res.form:{status:0};
            _this.bill = res.bill;
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("edit_chinese_orderid")
                if (res.orderid){
                  localStorage.setItem("edit_chinese_orderid",res.orderid);
      ...
      out(){
        this.delView('ChineseHerbalMedicine');
      },
    }
  }
</script>
