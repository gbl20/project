<template>
  <Modal v-model="show_self" title="选择商品功能类别" width="380" footer-hide :mask-closable="false">
        ...
        methods:{
            init:function(){
              ...
              _this.common.http({url:'w/getAddCfg',data: {}}).then(function(res){
                if(res.status==1) _this.data=res.function;
              }).catch(function(err){
          ...
          sel_column:function(data){
            ...
            this.sel_data={id:data[0].id,title:data[0].title};
          },
          /**
           * 确定选择
           */
          sure_cust:function () {
            ...
            this.$emit('confirm',this.sel_data);
            this.data=[];
