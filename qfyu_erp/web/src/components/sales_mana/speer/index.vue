    ...
    export default {
      name:'sel_speer',
        ...
        data(){
          return {
             height:0,
             Breadcrumb:[{title:'销售管理'},{title:'销售报价单'}],
             columns:[
        ...
        methods:{
          ...
          init:function(){
            let _this = this;
            this.loadding = true;
            this.common.http({url:'s/selSpeerList',data:{}}).then(function(res){
              if(res.status==1){
                _this.data = res.data;
              }
          ...
          addEdit:function(type){
            if(!type){
              localStorage.removeItem('speer_orderid');
              this.toUrl('/sel_speer_edit');
            }else {
              if(this.checkIndex != -1){
                localStorage.setItem('speer_orderid',this.data[this.checkIndex].orderid)
                this.toUrl('/sel_speer_edit');
              }
