    ...
    <split-none :Breadcrumb="Breadcrumb" :editTypeTitle="editTypeTitle" @DHeight="dHeight">
            ...
            <Col span="6">
              ...
              <FormItem label="验收员" prop="accept_code">
                <Row>
                  ...
                  <Col span="1">&nbsp;</Col>
                  <Col span="15">
                    <Input icon="ios-more" v-model="$store.state.emp[formData.accept_code]" @on-click="openEmployee"></Input>
                  </Col>
                </Row>
              </FormItem>
            </Col>
      ...
      <div slot="table-list">
        <Table border stripe size="small" :row-class-name="rowClassName" :columns="columns" :data="data"
          ...
          <template slot-scope="{row,index}" slot="entry_certificate">
            <Select v-if="index==editIndex && editType" v-model="data[index].entry_certificate">
              <Option v-for="(item,index) in $store.state.sysParam.check_default.entry_certificate" :value="index"
                      :key="index">{{item}}
              </Option>
            </Select>
            <span v-else>{{$store.state.sysParam.check_default.entry_certificate[row.entry_certificate]}}</span>
          </template>
  ...
  export default {
    ...
    data: function () {
      return {
        type: 3,
        orderid: null,
        Breadcrumb: [{title: '销售管理'}, {title: '销退质量验收'}, {title: '医疗器械质量验收'}],
        editIndex: -1,
        editType: 0,
        editTypeTitle: '',
        height: 0,
        ...
        columns: [
          ...
          {title: '入境货物检验检疫证明', slot: 'entry_certificate', width: 150, minWidth: 150, resizable: true, ellipsis: true},
        ],
        data: [],
        ruleInline: {
          accept_code: [
    ...
    activated() {
      this.orderid = localStorage.getItem("quality_gsp3_orderid");
    ...
    methods: {
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("quality_gsp3_orderid")
                if (res.orderid){
                  localStorage.setItem("quality_gsp3_orderid",res.orderid);
      ...
      out(){
        this.delView('SelQualityAcceptGsp3');
      },
    }
  }
</script>
