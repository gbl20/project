  ...
  export default {
    ...
    data: function () {
      return {
        type: 6,
        orderid: null,
        Breadcrumb: [{title: '销售管理'}, {title: '销退质量验收'}, {title: '非药品质量验收'}],
        editIndex: -1,
        editType: 0,
        editTypeTitle: '',
        height: 0,
    ...
    activated() {
      this.orderid = localStorage.getItem("quality_gsp6_orderid");
    },
    created() {
    ...
    methods: {
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("quality_gsp6_orderid")
                if (res.orderid){
                  localStorage.setItem("quality_gsp6_orderid",res.orderid);
      ...
      out(){
        this.delView('SelQualityAcceptGsp6');
      },
    }
  }
</script>
