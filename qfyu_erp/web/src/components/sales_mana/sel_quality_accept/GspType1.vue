  ...
  export default {
    ...
    data: function () {
      return {
        type: 1,
        orderid: null,
        Breadcrumb: [{title: '销售管理'}, {title: '销退质量验收'}, {title: '普通药品质量验收'}],
        editIndex: -1,
        editType: 0,
        editTypeTitle: '',
        height: 0,
    ...
    activated() {
      this.orderid = localStorage.getItem("quality_gsp1_orderid");
    },
    created() {
    ...
    methods: {
        ...
        this.$Modal.confirm({
          ...
          onOk(){
            _this.common.http({
              ...
              if (res.status) {
                localStorage.removeItem("quality_gsp1_orderid")
                if (res.orderid){
                  localStorage.setItem("quality_gsp1_orderid",res.orderid);
      ...
      selEmployee: function (e) {
        if (!this.state) this.formData.accept_code = e.code;
        else this.$set(this.data[this.editIndex],'accepter',e.code)
      },
      selPrice(e) {
      ...
      out(){
        this.delView('SelQualityAcceptGsp1');
      },
    }
  }
</script>
