    ...
    export default {
      name:'trans_reg_cold',
        ...
        data(){
          return {
             height:0,
             Breadcrumb:[{title:'冷藏药品运输'},{title:'冷藏药品运输登记查询'}],
             columns:[
              ...
              {title:'审核人',slot:'audit_no',width:120,align:'center'},
              {title:'已审核',slot:'is_transfer',width:120,align:'center'},
              {title:'送货司机',key:'driver',resizable:true,width:120,align:'center'},
              {title:'车牌号或温度仪表编号',key:'car_meter_no',width:120,resizable:true,align:'center'},
              {title:'送货登记单号',key:'pack_num',width:150,resizable:true,align:'center'},
              {title:'送货人',slot:'delivery_boy',width:100,resizable:true,align:'center'},
        ...
        methods:{
          ...
          init:function(){
            let _this = this;
            this.loadding = true;
            this.common.http({url:'s/transRegColdList',data:{}}).then(function(res){
              if(res.status==1){
                _this.data = res.data;
              }
          ...
          addEdit:function(type){
            if(!type){
              localStorage.removeItem('trans_reg_cold_orderid');
              this.toUrl('/trans_reg_cold_edit');
            }else {
              if(this.checkIndex != -1){
                localStorage.setItem('trans_reg_cold_orderid',this.data[this.checkIndex].orderid)
                this.toUrl('/trans_reg_cold_edit');
              }
