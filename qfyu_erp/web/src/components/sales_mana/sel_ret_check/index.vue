...
<div>
  <split-none :Breadcrumb="Breadcrumb" @DHeight="DHeight">
    <div slot="form-ser">
      <div class="nav-menu">
        <Button @click="addOrder"><span class="icon iconfont">&#xe797;</span>新增销退登记</Button>
        <Button @click="openSearch=true"><span class="icon iconfont">&#xe691;</span>筛选条件</Button>
    ...
    <Form :model="search" :label-width="80">
      <Row>
        <Col span="12">
          ...
          <FormItem label="制单人">
            <Input icon="ios-more" v-model="search.creater" @on-click="openEmp('creater')"></Input>
          </FormItem>
          <FormItem label="开单部门">
            <Input icon="ios-more" v-model="search.depart_id" @on-click="openDept"></Input>
          </FormItem>
        </Col>
        <Col span="12">
          <FormItem label="登记日期至">
            <DatePicker type="date" format="yyyy年-MM月-dd日" v-model="search.fend_date"></DatePicker>
          </FormItem>
          <FormItem label="客户编号">
            <Input icon="ios-more" v-model="search.cust_no" @on-click="openCust"></Input>
          </FormItem>
          <FormItem label="联系人">
            <Input icon="ios-more" v-model="search.linkman2" @on-click="openSales"></Input>
          </FormItem>
          <FormItem label="业务员">
            <Input icon="ios-more" v-model="search.emp_no" @on-click="openEmp('emp_no')"></Input>
          </FormItem>
          <FormItem label="审核人">
            <Input icon="ios-more" v-model="search.audit" @on-click="openEmp('audit')"></Input>
          </FormItem>
          <FormItem label="单据状态">
      ...
      <FormItem label="商品编号">
        <Row>
          <Col span="20"><Input icon="ios-more" v-model="search.goods_code" @on-click="openPro"></Input></Col>
...
<script>
  ...
  import SelectDepart from "@/components/common/SelectDepart"
    ...
    data(){
      return{
        height:0,
        Breadcrumb:[{title:'销售管理'},{title:'销售退回管理'},{title:'销售退回登记查询'}],
        columns:[
          {title: '单据编号', key: 'orderid', width: 155, minWidth: 155, ellipsis: true, resizable: true,fixed:'left'},
          {title: '申请日期', key: 'f_date', width: 140, minWidth: 140, ellipsis: true, resizable: true},
          {title: '合同号', key: 'contract_no', width: 120, minWidth: 120, ellipsis: true, resizable: true},
          {title: '发货单号', key: 'p_orderid', width: 155, minWidth: 155, ellipsis: true, resizable: true},
          ...
          {title: '复核人', key: 'audit', width: 70, minWidth: 70, ellipsis: true, resizable: true},
          {title: '复核人姓名', key: 'audit', width: 80, minWidth: 80, ellipsis: true, resizable: true},
          {title: '客户编号', key: 'cust_no', width: 80, minWidth: 80, ellipsis: true, resizable: true},
          {title: '客户简称', key: 'cust_name', width: 100, minWidth: 100, ellipsis: true, resizable: true},
          ...
          {title: '货款', key: 'goods_sum', width: 100, minWidth: 100, ellipsis: true, resizable: true,align:'right'},
          ...
          {title: '父级名称', key: 'cust_sname', width: 80, minWidth: 80, ellipsis: true, resizable: true},
        ...
        openSearch:false,
        search:{
          f_date:'',orderid:'',contract_no:'',goods_code:'',creater:'',depart_id:'',fend_date:'',cust_no:'',linkman2:'',emp_no:'',audit:'',status:2,
        },
        empName:'',
        status:0
      }
    ...
    methods:{
      ...
      addOrder(){
        ...
        this.toUrl('/SelRetCheckEdit');
      },
      changeRow(item,index,type){
        if (type){
          localStorage.setItem("sel_check_orderid",item.orderid);
          this.toUrl('/SelRetCheckEdit');
        }
      },
      changePage(page){
